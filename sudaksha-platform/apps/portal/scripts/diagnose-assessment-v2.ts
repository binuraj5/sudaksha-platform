 ...
 * 
 * Usage: pnpm tsx scripts/diagnose-assessment-v2.ts
 */
...
async function runDiagnostics() {
  try {
    ...
    const ASSESSMENT_ID = (selectedAssessment as any).assessment_id;
    const USER_ID = (selectedAssessment as any).user_id;

    log.data(`\n📌 Using assessment: ${ASSESSMENT_ID} (${(selectedAssessment as any).component_count} components)`);
    ...
    if (roleInfo && roleInfo.role_name) {
      ...
      log.error('⚠️  User has no assigned role');
      log.data('Cannot complete role-based analysis. Skipping queries 4 & 5.');
      
      // ========== SUMMARY without role analysis ==========
      log.title('DIAGNOSTIC SUMMARY (Assessment Only)');
...
📈 Competencies Assessed:
   - Total components completed: ${assessedCount}
   - Competencies covered: ${assessedCount}

Note: Role-based gap analysis requires a user role assignment.
      `);
    ...
    const query4 = `
      ...
      SELECT 
        ...
        uac.percentage AS achieved_percentage,
        ROUND(COALESCE(uac.score, 0)::numeric, 1) AS achieved_score,
        COALESCE(uac.status, 'NOT_STARTED') AS component_status,
        CASE WHEN uac.id IS NOT NULL THEN 'ASSESSED' ELSE 'NOT ASSESSED' END AS assessed_flag
      FROM (SELECT * FROM user_role) ur
      LEFT JOIN "RoleCompetency" rc ON rc."roleId" = ur."assignedRoleId"
      LEFT JOIN "Competency" c ON c.id = rc."competencyId"
      LEFT JOIN "AssessmentModelComponent" amc ON amc."competencyId" = c.id 
        AND amc."modelId" = (SELECT "modelId" FROM "UserAssessmentModel" WHERE id = $1)
    ...
    const result4 = await pool.query(query4, [ASSESSMENT_ID]);
    ...
    if (comparisonData.length > 0) {
      ...
      log.error('No role competencies found to compare');
    }
