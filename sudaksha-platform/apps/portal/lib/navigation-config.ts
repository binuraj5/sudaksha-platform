...
const basePathOrPortal = (base: string, portal: string, segment: string) => {
    // For "My Personal Page" items, we want to force them to the individual portal routes 
    // rather than the org routes, so they see their personal view.
    return portal;
};

const MY_PAGE_NAV_ITEMS: NavigationItem[] = [
    { id: 'my-dashboard', icon: Home, label: 'My Dashboard', path: (base) => basePathOrPortal(base, '/assessments/individuals/dashboard', '/dashboard'), permission: '*', roles: ALL_PROFILE_ROLES },
    ...
    { id: 'my-hierarchy', icon: Users, label: 'My Hierarchy', path: (base) => basePathOrPortal(base, '/assessments/individuals/hierarchy', '/hierarchy'), permission: '*', roles: ROLES_WITH_HIERARCHY },
    {
        id: 'my-career',
        icon: Briefcase,
        label: 'My Career',
    ...
    { id: 'take-survey', icon: ClipboardList, label: 'Take Survey', path: (base) => basePathOrPortal(base, '/assessments/individuals/surveys', '/surveys'), permission: '*', roles: ALL_PROFILE_ROLES },
    { id: 'my-curriculum', icon: BookOpen, label: 'My Curriculum', path: (base) => basePathOrPortal(base, '/assessments/individuals/curriculum', '/curriculum'), permission: '*', roles: ALL_PROFILE_ROLES, tenantTypes: ['INSTITUTION'] },
