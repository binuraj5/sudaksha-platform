...
import { createRoleWithCompetencies, requestRoleWithCompetencies } from "@/lib/actions/role-actions";
...
interface CreateRoleWizardProps {
    ...
    mode?: "CREATE" | "REQUEST";
    onSuccess?: (id?: string) => void; // Optional callback for modals
}

export function CreateRoleWizard({ returnUrl, mode = "CREATE", onSuccess }: CreateRoleWizardProps = {}) {
    ...
    const handleSave = async () => {
        try {
            ...
            let res;
            if (mode === "REQUEST") {
                res = await requestRoleWithCompetencies(payload, competencies);
            } else {
                res = await createRoleWithCompetencies(payload, competencies);
            }

            if (res.success) {
                toast.success(mode === "REQUEST" ? "Role request submitted successfully!" : "Role created successfully!");
                if (onSuccess) {
                    onSuccess((res as any).roleId || (res as any).requestId);
                }
                if (returnUrl) {
                    router.push(returnUrl);
                }
            } else {
                toast.error(`Failed to ${mode.toLowerCase()} role: ` + res.error);
            }
        } catch (err) {
                    ...
                    {currentStep === 3 && (
                        <div className="text-center py-10">
                            <h3 className="text-lg font-medium text-slate-900">
                                {mode === "REQUEST" ? "Ready to Request Role" : "Ready to Create Role"}
                            </h3>
                            <p className="mt-2 text-sm text-slate-500">
                                You are about to {mode === "REQUEST" ? "request" : "create"} the role <strong>{formData.name}</strong> with {competencies.length} competencies.
                                {mode === "REQUEST" && " An administrator will process your request before it becomes active in your profile."}
                            </p>
            ...
            <div className="mt-8 flex justify-between">
                ...
                {currentStep === 3 ? (
                    <Button onClick={handleSave} className="bg-green-600 hover:bg-green-700">
                        {mode === "REQUEST" ? "Submit Request" : "Create Role"}
                    </Button>
                ) : (
