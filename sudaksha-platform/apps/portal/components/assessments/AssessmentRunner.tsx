...
interface AssessmentRunnerProps {
    userAssessment: any;
    /** Optional: section index to resume from (0-based). Defaults to 0. */
    initialSectionIndex?: number;
}

type RunnerStep = 'INSTRUCTIONS' | 'COMPONENT_INTRO' | 'COMPONENT_RUNNING' | 'SECTION_CHECKPOINT' | 'FINISHED';
...
export function AssessmentRunner({ userAssessment, initialSectionIndex = 0 }: AssessmentRunnerProps) {
    ...
    const [activeComponentIndex, setActiveComponentIndex] = useState(initialSectionIndex);
    ...
    // ── Between-sections checkpoint screen ─────────────────────────────────
    if (step === 'SECTION_CHECKPOINT') {
        const nextComponent = components[activeComponentIndex + 1] ?? null;
        return (
            <SectionCheckpoint
                completedSection={{
                    name: lastCompletedSection?.name ?? (activeComponent as { name?: string })?.name ?? `Section ${activeComponentIndex + 1}`,
                    componentType: lastCompletedSection?.componentType ?? (activeComponent as { componentType?: string })?.componentType ?? "",
                    score: lastCompletedSection?.score ?? null,
                    percentage: lastCompletedSection?.percentage ?? null,
                    timeSpentSeconds: lastCompletedSection?.timeSpentSeconds ?? null,
                }}
                nextSection={nextComponent ? {
                    name: (nextComponent as { name?: string })?.name ?? `Section ${activeComponentIndex + 2}`,
                    componentType: (nextComponent as { componentType?: string })?.componentType ?? "",
                    estimatedMinutes: (nextComponent as { customDuration?: number })?.customDuration ?? 15,
                } : null}
                sectionNumber={activeComponentIndex + 1}
                totalSections={components.length}
                loading={loading}
                onReview={() => {
                    // Review: go back to the completed section in read-only mode
                    setStep("COMPONENT_RUNNING");
                }}
                onContinue={() => {
                    // Continue: advance to next section
                    setActiveComponentIndex((i) => i + 1);
                    setStep("COMPONENT_INTRO");
                }}
            />
    ...
    if (step === "COMPONENT_RUNNING") {
        ...
        const handleNextSection = async () => {
            ...
            try {
                ...
                // Track last completed section for checkpoint screen
                setLastCompletedSection({
                    name: (activeComponent as { name?: string })?.name ?? `Section ${activeComponentIndex + 1}`,
                    componentType: (activeComponent as { componentType?: string })?.componentType ?? "",
                    score: null,
                    percentage: null,
                ...
                if (data.isLastSection) {
                    ...
                    // Show checkpoint screen instead of jumping straight to next section
                    setStep("SECTION_CHECKPOINT");
                }
            } catch (e) {
