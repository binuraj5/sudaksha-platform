...
import { PendingRoleRequestsTab } from "./PendingRoleRequestsTab";
        ...
        <>
            ...
            <div className="space-y-6">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    {activeTab !== "request_role" && activeTab !== "my_requests" && activeTab !== "pending_review" && (
                    ...
                    <TabsContent value="pending_review">
                        <PendingRoleRequestsTab clientId={clientId} />
                    </TabsContent>
                    ...
                    <TabsContent value="user_requests">
                        <UserRoleRequestsTab clientId={clientId} />
                    </TabsContent>
                </Tabs>
            </div>
        </>
...
function UserRoleRequestsTab({ clientId }: { clientId?: string }) {
    ...
    const fetchRequests = async () => {
        setLoading(true);
        try {
            // Note: If you want to scope this explicitly to a clientId in the future, you can branch here.
            // Currently, the admin API scopes by the current user's tenant if they are an org admin.
            const res = await fetch("/api/admin/role-assignment-requests");
            if (!res.ok) throw new Error("Failed to fetch user role requests");
            const data = await res.json();
            setRequests(data.requests || []);
        } catch (error) {
            toast.error("Failed to load user role requests");
            console.error(error);
        } finally {
            setLoading(false);
        }
    ...
    if (requests.length === 0) {
        return (
            <div className="bg-white rounded-lg border border-slate-200 text-center py-12 text-slate-500">
                No pending user role assignments found.
            </div>
        );
    }
            ...
            <Table>
                ...
                <TableBody>
                    {requests.map((req) => (
                        <TableRow key={req.id}>
                            <TableCell>
                                <div className="font-medium text-slate-900">{req.member?.name || "Unknown"}</div>
                                <div className="text-xs text-slate-500">{req.member?.email}</div>
                                {req.tenant?.name && (
                                    <div className="text-xs text-indigo-500 mt-1">{req.tenant.name}</div>
                                )}
                            </TableCell>
                            <TableCell>
                                <div className="font-medium">{req.requestedRoleName}</div>
                                {req.description && (
