...
export async function POST(
    ...
    // Frontend sends { decision, notes } — support both "notes" and legacy "reason"
    ...
    try {
        ...
        if (isAssessmentsDB) {
            // It's an assessment request stored in db-assessments.
            ...
            // It's a role or competency request stored in db-core.
            result = await prisma.$transaction(async (tx) => {
                ...
                if (decision === "APPROVED") {
                    if (request!.type === "ROLE") {
                        if (isNewRoleRequest) {
                            ...
                            // Create competencies attached to the role from originalData
                            ...
                            for (const comp of competencies) {
                                ...
                                // Link the competency to the role
                            ...
                            // Assign the approved role to the requester
                            if (request!.requesterId) {
                                ...
                                if (requesterUser?.email) {
                                    const member = await tx.member.findFirst({
                                        where: { email: requesterUser.email, ...(request!.tenantId ? { tenantId: request!.tenantId } : {}) },
                                    });
                                    if (member) {
                            ...
                            // Existing role — just mark it approved
                ...
                const updatedRequest = await tx.approvalRequest.update({
                    ...
                    data: {
                        ...
                        reviewerId: u.id,
                        reviewedAt: new Date(),
                        // If a new role was created, update entityId to the real role ID
