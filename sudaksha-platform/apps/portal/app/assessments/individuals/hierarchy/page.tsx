...
export default async function IndividualHierarchyPage() {
    ...
    if (!member || !member.tenantId) {
        return (
            <div className="p-8 max-w-7xl mx-auto">
                ...
                <p className="text-gray-500">Member profile not found or you do not belong to an organization.</p>
            </div>
        );
    }

    const labels = await resolveTenantLabels(member.tenantId);
    ...
    const Node = ({
        ...
        <Card
            className={`w-full max-w-md mx-auto ${highlight ? "border-2 border-indigo-500 bg-indigo-50 shadow-md" : "border-gray-200"}`}
        >
            <CardContent className="p-4 flex items-center gap-4">
                <div
                    className={`h-10 w-10 rounded-full flex items-center justify-center ${highlight ? "bg-indigo-200 text-indigo-700" : "bg-gray-100 text-gray-500"}`}
                >
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8 text-center">
            ...
            <div className="space-y-0">
                <Node icon={Building} title={member.tenant?.name || "Organization"} subtitle="Organization" />
                <Connector />
                ...
                <Node icon={User} title={member.name} subtitle={member.designation || "Member"} highlight />
            </div>
        </div>
    );
}
