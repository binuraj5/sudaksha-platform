...
export default async function ProjectsPage({
    ...
    const formatted = projects.map(p => ({
        ...
        stats: {
            ...
            progress: 35 // Mock progress for visualization
        },
        ...
        <div className="p-8 h-full flex flex-col max-w-[1600px] mx-auto">
            <header className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-gray-500 font-medium">Manage and track {labels.activityPlural.toLowerCase()}</p>
                </div>
            ...
            <Tabs defaultValue="board" className="flex-1 flex flex-col">
                ...
                <TabsContent value="board" className="flex-1 overflow-x-auto pb-4">
                    <div className="flex gap-4 min-w-[1000px] h-full">
                        {columns.map(col => (
                            <div key={col.id} className="flex-1 min-w-[280px] bg-gray-50/50 rounded-xl border p-4 flex flex-col">
                                <div className="flex justify-between items-center mb-4">
                                ...
                                <div className="space-y-3 flex-1 overflow-y-auto pr-2">
                                    {formatted.filter(p => p.status === col.id).map(p => (
