...
async function getCybersecurityCourses() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/courses?technologyDomain=cybersecurity&limit=12`,
      { next: { revalidate: 60 } }
    );
    if (!res.ok) return [];
...
export default async function CybersecurityPage() {
  const courses = await getCybersecurityCourses();
