...
async function getDataAnalyticsCourses() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/courses?technologyDomain=data-analytics&limit=12`,
      { next: { revalidate: 60 } }
    );
    if (!res.ok) return [];
...
export default async function DataAnalyticsPage() {
  const courses = await getDataAnalyticsCourses();
