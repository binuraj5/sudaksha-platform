...
async function getPersonalDevelopmentCourses() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/courses?categoryPrimary=PERSONAL_DEVELOPMENT&limit=12`,
      { next: { revalidate: 60 } }
    );
    if (!res.ok) return [];
...
export default async function PersonalDevelopmentPage() {
  const courses = await getPersonalDevelopmentCourses();

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Personal Development</h1>
