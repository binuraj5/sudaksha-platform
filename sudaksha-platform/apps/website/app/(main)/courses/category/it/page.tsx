...
async function getITCourses() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/courses?categoryPrimary=IT&limit=12`,
      { next: { revalidate: 60 } }
    );
    if (!res.ok) return [];
...
export default async function ITCoursesPage() {
  const courses = await getITCourses();
