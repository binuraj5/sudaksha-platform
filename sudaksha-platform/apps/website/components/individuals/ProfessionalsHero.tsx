...
import { ArrowRight, Briefcase, TrendingUp, Clock, DollarSign, Award } from 'lucide-react';
...
export default function ProfessionalsHero({ data }: ProfessionalsHeroProps) {
      ...
      <div className="relative z-10 pt-4 pb-2">
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <span>/</span>
            <span className="text-sudaksha-blue-600 font-medium">Working Professionals</span>
          </motion.div>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            >
              {data.valuePropositions.map((prop: any, index: number) => {
                ...
                const IconComponent = iconMap[firstWord as keyof typeof iconMap] || Briefcase;
          ...
          >
            <div className="relative aspect-square lg:aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl">
              {/* Career Stages Grid */}
              <div className="absolute inset-0 grid grid-rows-2 gap-2 bg-sudaksha-navy-100 p-4">
                {/* Stage 1: Junior/Mid-Level */}
                <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-blue-500 to-blue-700">
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center text-white p-4">
                      ...
                      <h3 className="text-lg font-bold mb-1">{data.careerStages[0].title}</h3>
                      <p className="text-xs opacity-90 mb-2">{data.careerStages[0].experience}</p>
                      <p className="text-xs opacity-75">{data.careerStages[0].programs} Programs</p>
                    </div>
                  </div>
                </div>

                {/* Stage 2: Senior/Leadership */}
                <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-purple-500 to-purple-700">
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center text-white p-4">
                      ...
                      <h3 className="text-lg font-bold mb-1">{data.careerStages[1].title}</h3>
                      <p className="text-xs opacity-90 mb-2">{data.careerStages[1].experience}</p>
                      <p className="text-xs opacity-75">{data.careerStages[1].programs} Programs</p>
                    </div>
                  </div>
                </div>
              </div>
