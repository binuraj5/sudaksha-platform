            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                    <Button variant="outline" asChild>
                        <Link href={`/assessments/clients/${clientId}/assessments`}>Save for Later</Link>
                    </Button>
                    <Button className="bg-indigo-600 hover:bg-indigo-700" asChild>
                        <Link href={`/assessments/clients/${clientId}/assessments/${modelId}/builder`}>
                ...
                <div className="lg:col-span-3 space-y-6">
                    <Tabs value={mode} onValueChange={(v: any) => setMode(v)} className="w-full">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            {mode === "list" && (
                                <Button className="h-12 rounded-xl bg-slate-800 hover:bg-slate-900 font-bold gap-2 text-xs" onClick={() => setMode("manual")}>
                        ...
                        <TabsContent value="list" className="mt-0 focus-visible:ring-0">
                            <QuestionList questions={questions} indicators={indicators} onEdit={() => toast.info("Edit coming soon")} onDelete={handleDeleteQuestion} onDuplicate={(q) => { const { id, ...rest } = q; handleSaveQuestion(rest); }} />
                        </TabsContent>
                        <TabsContent value="manual" className="mt-0 focus-visible:ring-0">
                            <Card className="border-none shadow-2xl rounded-[2.5rem] bg-white/80 overflow-hidden">
                                <CardContent className="p-10">
                                    <QuestionForm componentId={selectedComponentId!} indicators={indicators} onSave={handleSaveQuestion} onCancel={() => setMode("list")} />
                                </CardContent>
                            </Card>
                        </TabsContent>
