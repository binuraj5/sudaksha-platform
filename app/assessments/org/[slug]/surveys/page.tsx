...
export default function OrgSurveysPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    // Resolve clientId from slug via API
    const [clientId, setClientId] = useState<string | null>(null);
    useEffect(() => {
        fetch(`/api/org/${slug}/tenant`)
            .then((r) => r.json())
            .then((t) => t?.id && setClientId(t.id))
            .catch(() => {});
    }, [slug]);

    useEffect(() => {
        if (view === "LIST" && clientId) {
            fetchSurveys();
        }
    }, [view, clientId]);
                            ...
                            <CardContent className="space-y-3">
                                <div className="flex flex-wrap gap-2">
                                    <Button variant="outline" size="sm" className="gap-1" asChild>
                                        <Link href={`${basePath}/surveys/${survey.id}/edit`}>Edit questions</Link>
                                    </Button>
                                    <Button variant="outline" size="sm" className="gap-1" asChild>
                                        <Link href={`${basePath}/surveys/${survey.id}/results`}>View results</Link>
                                    </Button>
