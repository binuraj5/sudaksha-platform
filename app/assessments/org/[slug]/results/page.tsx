...
export default async function OrgResultsPage({
    ...
    const completedAssessments = await prisma.projectUserAssessment.findMany({
        where: {
    ...
    return (
        <div className="p-8 space-y-8 max-w-7xl mx-auto">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">Assessment Scores</h1>
            ...
            {completedAssessments.length === 0 ? (
                <Card className="py-20 text-center border-dashed">
                    <CardContent>
                        ...
                        <Button asChild className="mt-6 bg-blue-600" variant="secondary">
                            <Link href={`${basePath}/assessments`}>Go to Assessments</Link>
                        </Button>
                    </CardContent>
                </Card>
            ) : (
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {completedAssessments.map((a) => (
                                <TableRow key={a.id}>
                                    <TableCell>
                                        <div className="font-medium text-gray-900">{a.projectAssignment.model.name}</div>
                                        <div className="text-[10px] text-gray-500 uppercase">Assessment</div>
                                    </TableCell>
                                    <TableCell className="text-sm text-gray-600">
                                        {a.projectAssignment.project.name}
                                    </TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell className="text-sm text-gray-500 font-mono">
                                        {a.completedAt ? formatDate(a.completedAt) : "Missing date"}
                                    </TableCell>
                                    <TableCell>
                                        <Button variant="ghost" size="sm" asChild>
                                            <Link href={`${basePath}/results/${a.id}`}>
                                                View Report
