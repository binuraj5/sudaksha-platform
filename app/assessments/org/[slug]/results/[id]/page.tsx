...
export default async function OrgResultDetailPage({
    params,
}: {
        ...
        if (member) {
            ...
            if (memberAssessment) {
                ...
                if (uam) {
                    ...
                    const totalTimeSpent = componentResults.reduce(
                        (s: number, r: { timeSpent?: number | null }) => s + (r.timeSpent ?? 0),
                        0
                    );
                    assessment = {
    ...
    const backHref = isB2C ? "/assessments/individuals/dashboard" : `${basePath}/results`;
            ...
            <header className="flex items-center gap-4">
                <Button variant="ghost" size="sm" asChild className="-ml-2">
                    <Link href={backHref}>
                        ...
                        {isB2C ? "Back to Dashboard" : "Back to Assessment Scores"}
                    </Link>
                </Button>
            </header>
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-1">
                                <h4 className="font-bold text-gray-900">
                                    {assessment.projectAssignment?.model?.name ?? "Assessment"}
                                </h4>
                                <p className="text-xs text-gray-400 line-clamp-2">
                                    {assessment.projectAssignment?.model?.description ?? ""}
                                </p>
                            </div>
                            <div className="pt-4 border-t space-y-3">
                                ...
                                <div className="flex justify-between text-xs">
                                    ...
                                    <span className="font-bold text-gray-700">
                                        {assessment.totalTimeSpent != null
                                            ? `${Math.round(Number(assessment.totalTimeSpent) / 60)} mins`
                                            : "—"}
                                    </span>
                                </div>
                            </div>
                                        ...
                                        <Card key={res.id} className="hover:border-red-100 transition-colors shadow-sm">
                                            <CardContent className="p-4 flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    <div
                                                        className={`p-2 rounded-lg ${res.status === "COMPLETED" ? "bg-green-50" : "bg-gray-50"}`}
                                                    >
                                                        <CheckCircle2
                                                            className={`h-5 w-5 ${res.status === "COMPLETED" ? "text-green-600" : "text-gray-300"}`}
                                                        />
                                                    </div>
                                                    <div>
                                                        <h4 className="font-bold text-sm text-gray-900">
                                                            {res.component?.competency?.name ??
                                                                `Section ${(res.component?.order ?? 0) + 1}`}
                                                        </h4>
                                                        <Badge
                                                            variant="outline"
                                                            className="text-[10px] uppercase border-none bg-gray-100 text-gray-500"
                                                        >
                                                ...
                                                <div className="text-right">
                                                    <p className="text-lg font-black text-gray-900">
                                                        {Math.round(res.percentage ?? res.score ?? 0)}%
                                                    </p>
